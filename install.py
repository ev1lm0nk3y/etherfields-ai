...
def check_prerequisites():
    ...
    if major < 3 or (major == 3 and minor != 11):
        print_error(f"Python 3.11 is required. Detected Python {major}.{minor}")
        sys.exit(1)
    ...
    if not uv_path:
        print_error(
            "`uv` was not found in the system PATH. Please install it first (https://github.com/astral-sh/uv)."
        )
        sys.exit(1)
    ...
    if not gemini_path:
        print_warning(
            "`gemini` CLI was not found in the system PATH. (https://geminicli.com/docs/get-started/installation/#install-gemini-cli)"
        )
    else:
...
def setup_directories_and_env():
    ...
    default_dir = "~/.local/etherfields-ai"
    custom_dir_str = ask_input(
        "Define the custom files directory to store AI agent files", default_dir
    )
    ...
    # Create directories
    subdirs = ["logs", "rulebook_pages", "topics", "voice/models", "voice/cache", "voice/train"]
    for subdir in subdirs:
        dir_path = custom_dir / subdir
        dir_path.mkdir(mode=755, parents=True, exist_ok=True)
        print_success(f"Ensured directory: {dir_path}")

    # 2. Ask for Voice Assist enablement
    enable_voice = ask_yes_no(
        "Would you like to enable the voice/TTS and wake-word listener capabilities?", default=False
    )
    ...
    if enable_voice:
        ...
        if not ffmpeg_path:
            ...
            print_warning(
                "Please install ffmpeg on your system first (e.g. `brew install ffmpeg` on macOS) or proceed without voice."
            )
            sys.exit(1)
    ...
    if enable_voice:
        wake_word_choice = ask_input(
            "Enter path to a custom .onnx or .tflite wake-word model, or press Enter to download 'jarvis'",
            "jarvis",
        )
        if wake_word_choice.lower() == "jarvis":
            ...
            if not target_path.exists():
                ...
                try:
                    ...
                    print_warning(
                        "You may need to manually place jarvis.onnx in your models directory."
                    )
            else:
            ...
            if src_model_path.exists() and src_model_path.suffix in [".onnx", ".tflite"]:
                ...
                print_success(
                    f"Copied custom wake word model {src_model_path.name} to {target_path}"
                )
            else:
                print_error(f"File {wake_word_choice} does not exist or is not a valid model file.")
                print_warning(
                    "Falling back to default 'jarvis.onnx'. Please place your model inside custom directory manually."
                )
                wake_word_model = "jarvis.onnx"
...
ETHERFIELDS_LOCAL_PATH={custom_dir_str}
...
def handle_rulebook_pdf(custom_dir):
    ...
    if not custom_pdf_path.exists():
        ...
        pdf_input = ask_input(
            "Enter local path to the Rulebook v2.0 PDF, or a URL to download it", default_url
        )

        if pdf_input.startswith("http://") or pdf_input.startswith("https://"):
            print_info("Downloading rulebook 2.0 from Awaken Realms...")
            try:
                ...
                print_warning(
                    f"Please manually download the rulebook v2.0 and save it as {custom_pdf_path}"
                )
        else:
            ...
            if user_pdf.exists():
                ...
                print_warning(f"Please manually place your PDF rulebook v2.0 into {custom_dir}")
...
def detect_hardware():
    ...
    if sys.platform == "darwin":
        try:
            ...
            if (
                "apple" in brand.lower()
                or "m1" in brand.lower()
                or "m2" in brand.lower()
                or "m3" in brand.lower()
            ):
                return "mps"
        except Exception:
            pass
...
def sync_environment(target):
    print_info(
        f"Synchronizing environment and installing developer tools via `uv sync` ({target.upper()} optimized)..."
    )
    try:
        if target == "cuda":
            cmd = [
        ...
        print_success(
            "Project environment synced successfully (dependencies and dev tools installed)."
        )
    except subprocess.CalledProcessError as e:
